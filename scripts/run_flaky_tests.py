...
def run_diag_helper(outdir: Path):
    ...
    try:
        ...
        p = subprocess.run([PY, str(diag)], capture_output=True, text=True, env=env, check=False)
        (outdir / 'diag_start_adapter.log').write_text(p.stdout + '\n---stderr---\n' + p.stderr)
    except Exception as e:
        (outdir / 'diag_start_adapter.log').write_text(f'failed running diag helper: {e}')


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--nodeids', nargs='+', required=True, help='pytest node ids to run')
    ap.add_argument('--outdir', default='flake-logs', help='directory to store logs')
    ap.add_argument('--max-iterations', type=int, default=1000, help='max iterations (safety)')
    args = ap.parse_args()
