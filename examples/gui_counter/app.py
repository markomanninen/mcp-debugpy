...
class CounterApp(ttk.Frame):
    def __init__(self, master: tk.Tk | None = None):
        ...
        self.value_label = ttk.Label(self, textvariable=self.value_var, font=("Menlo", 18))
        self.value_label.pack(anchor="center", pady=(0, 12))

        btn_row = ttk.Frame(self)
        btn_row.pack()
        ttk.Button(btn_row, text="Increment", command=self.on_increment).pack(side=tk.LEFT, padx=4)
        ttk.Button(btn_row, text="Decrement", command=self.on_decrement).pack(side=tk.LEFT, padx=4)
        ttk.Button(btn_row, text="Reset", command=self.on_reset).pack(side=tk.LEFT, padx=4)
