...
async def main():
    ...
    c = StdioDAPClient()
    await c.start()
    print("   [OK] Adapter started\n")

    # Step 1: Initialize
    print("2. Sending initialize request...")
    init_resp = await c.initialize()
    print(f"   [OK] Initialized (success={init_resp.get('success')})\n")
    ...
    try:
        ...
        print("   [OK] Received 'initialized' event\n")
    except asyncio.TimeoutError:
        print("   [WARN] No 'initialized' event yet; continuing with configuration\n")

    async def wait_for_initialized_retry(reason: str, timeout: float = 5.0) -> bool:
        if c.initialized_received():
            return True
        print(f"   [RETRY] Waiting for 'initialized' ({reason})...")
        try:
            await c.wait_for_initialized(timeout=timeout)
            print(f"   [OK] 'initialized' event received ({reason})")
            return True
        except asyncio.TimeoutError:
            print(f"   [WARN] Still no 'initialized' event ({reason})")
            return False
    ...
    print(f"   [OK] Breakpoint set at {APP}:{bp_line}")
    ...
    if (
        ...
        if waited:
            bp_resp = await c.setBreakpoints(str(APP), [bp_line])
            print(f"   [RETRY] Retry response: {bp_resp}")
    print()
    ...
    print(
        f"   [OK] Exception breakpoints response (success={exc_resp.get('success', True)})"
    )
    ...
    if (
        ...
        if waited:
            exc_resp = await c.setExceptionBreakpoints([])
            print(f"   [RETRY] Retry response: {exc_resp}\n")
    ...
    print(f"   [OK] Configuration done (success={cfg_success})")
    ...
    if post_init_ready:
        if not bp_resp.get("success", True):
            print("   [RETRY] Re-sending setBreakpoints after 'initialized'...")
            bp_resp = await c.setBreakpoints(str(APP), [bp_line])
            print(f"     response: {bp_resp}")
        if not exc_resp.get("success", True):
            print(
                "   [RETRY] Re-sending setExceptionBreakpoints after 'initialized'..."
            )
            exc_resp = await c.setExceptionBreakpoints([])
            print(f"     response: {exc_resp}")
    print()
    ...
    print(f"   [OK] Launch response (success={launch_resp.get('success')})\n")
    ...
    try:
        ...
        print(f"   [OK] Stopped event received (reason={reason})")
    except asyncio.TimeoutError:
        print("   [WARN] Timeout waiting for 'stopped' event; polling threads directly")
        await asyncio.sleep(0.5)

    # Capture any other events while we are paused.
    print("   [INFO] Polling adapter event queue for additional context (1s)...")
    extra_events = []
    ...
    if locals_ref:
        ...
        print("   [ERROR] No locals scope found\n")
    ...
    frames = st.get("body", {}).get("stackFrames", [])
    if frames:
        print(f"    [OK] Now at line {frames[0].get('line')}\n")
    ...
    try:
        ...
        frames = st.get("body", {}).get("stackFrames", [])
        if frames:
            print(f"    [OK] Step-in reached line {frames[0].get('line')}")
    except Exception as exc:
        print(f"    [WARN] Step-in not available: {exc}")
    ...
    try:
        ...
        frames = st.get("body", {}).get("stackFrames", [])
        if frames:
            print(f"    [OK] Step-out reached line {frames[0].get('line')}")
    except Exception as exc:
        print(f"    [WARN] Step-out not available: {exc}")
    ...
    await c.close()
    print("\n[SUCCESS] Direct adapter walkthrough complete")
